...
of the page, in order, with the canvas's own markup, values and copy.
...
Astro, static output, one page: `src/pages/index.astro`. No component library,
no CSS framework, no design tokens — the canvas's inline styles are the
styles. The 3D scene is `public/design/hands-scene.html` (Three.js from unpkg,
pinned with integrity hashes, as the canvas has it) in an iframe; the
background fields are `public/design/mh-bg.js`, a dependency-free custom
element. Fonts are Literata and Space Grotesk from Google Fonts, as the canvas
loads them.

Old routes redirect to the page's anchors (`astro.config.mjs`).
...
2. Re-transcribe: the mapping from canvas to page is documented at the top
